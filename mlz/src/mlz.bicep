...
module securityNamingConvention 'modules/naming-convention.bicep' = {
  name: 'naming-convention-security-${deploymentNameSuffix}'
  params: {
    delimiter: '-'
    environmentAbbreviation: environmentAbbreviation
    identifier: identifier
    locationAbbreviation: networking.outputs.locationProperties.abbreviation
    networkName: 'security'
    resourceAbbreviations: networking.outputs.resourceAbbreviations
  }
}

var securityResourceGroupName = empty(sentinelResourceGroupName)
  ? securityNamingConvention.outputs.names.resourceGroup
  : sentinelResourceGroupName

module securityResourceGroup 'modules/resource-group.bicep' = {
  name: 'deploy-rg-security-${deploymentNameSuffix}'
  scope: subscription(operationsSubscriptionId)
  params: {
    enabled: deploySentinel
...
module sentinelContent 'modules/sentinel-content.bicep' = if (deploySentinel) {
  ...
  params: {
    ...
    workspaceSubscriptionId: operationsTier.subscriptionId
...
module diagnosticSettings 'modules/diagnostic-settings.bicep' = {
  ...
  params: {
    enableActivityLogDiagnosticSettings: deploySentinel && enableAzureActivityDataConnector
    defaultActivityLogDiagnosticSettingName: azureActivityDiagnosticSettingName
    activityLogDiagnosticSettingNames: activityLogDiagnosticSettingNames

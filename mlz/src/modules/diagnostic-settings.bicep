...
@description('Toggle to deploy subscription Activity Log diagnostic settings into the Log Analytics workspace.')
param enableActivityLogDiagnosticSettings bool = true

@description('Optional map of subscriptionId => Activity Log diagnostic setting name. Used to adopt an existing diagnostic setting name when redeploying.')
param activityLogDiagnosticSettingNames object = {}

@description('Default Activity Log diagnostic setting name to use when no per-subscription override exists. Leave blank to allow the nested module to apply its default naming.')
param defaultActivityLogDiagnosticSettingName string = ''
...
module activityLogDiagnosticSettings 'activity-log-diagnostic-settings.bicep' = [for (subscriptionId, i) in dedupedSubscriptionIds: if (enableActivityLogDiagnosticSettings) {
  name: 'deploy-activity-diags-${i}-${deploymentNameSuffix}'
  scope: subscription(subscriptionId)
  params: {
    logAnalyticsWorkspaceId: logAnalyticsWorkspaceResourceId
    diagnosticSettingName: contains(activityLogDiagnosticSettingNames, subscriptionId)
      ? string(activityLogDiagnosticSettingNames[subscriptionId])
      : defaultActivityLogDiagnosticSettingName
  }
}]
...
module logAnalyticsWorkspaceDiagnosticSetting 'log-analytics-diagnostic-setting.bicep' = {
  ...
  params: {
    ...
    diagnosticStorageAccountResourceGroupName: split(storageAccountResourceIds[1], '/')[4]
